...
static void check_bound(IOMap *map, paddr_t addr)
{
    if (map == NULL)
    {
        Assert(map != NULL, "address (" FMT_PADDR ") is out of bound at pc = " FMT_WORD, addr, TOP_PC);
    }
    else
    {
        Assert(addr <= map->high && addr >= map->low, "address (" FMT_PADDR ") is out of bound {%s} [" FMT_PADDR ", " FMT_PADDR "] at pc = " FMT_WORD, addr, map->name, map->low, map->high, TOP_PC);
    }
}
...
void map_write(paddr_t addr, int len, word_t data, IOMap *map)
{
    ...
    paddr_t offset = addr - map->low;
#ifdef CONFIG_DTRACE
    dtrace_record(false, addr, len, map, host_read((uint8_t *)map->space + offset, len), data);
#endif
