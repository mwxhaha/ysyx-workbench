...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)                             \
    ...
    INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal, J, R(rd) = s->pc + 4; s->dnpc = s->pc + imm; IFDEF(CONFIG_FTRACE, ftrace_record(s);));
    INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr, I, word_t t = s->pc + 4; s->dnpc = (src1 + imm) & 0xfffffffe; R(rd) = t; IFDEF(CONFIG_FTRACE, ftrace_record(s);));
