...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)         \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 00101 11", auipc, U,
          R(rd) = (sword_t)s->pc + (sword_t)imm);
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal, J,
          R(rd) = s->pc + (word_t)4;
          s->dnpc = (sword_t)s->pc + (sword_t)imm; ftrace_record(s,0););
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr, I,
          word_t t = s->pc + (word_t)4;
          s->dnpc = ((sword_t)src1 + (sword_t)imm) & 0xfffffffe; R(rd) = t;
          if (s->isa.inst.val == 0x00008067) ftrace_record(s,1));
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq, B,
          if (src1 == src2) s->dnpc = (sword_t)s->pc + (sword_t)imm);
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne, B,
          if (src1 != src2) s->dnpc = (sword_t)s->pc + (sword_t)imm);
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt, B,
          if ((sword_t)src1 < (sword_t)src2) s->dnpc =
              (sword_t)s->pc + (sword_t)imm);
  ...
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh, I,
          R(rd) = SEXT(Mr((sword_t)src1 + (sword_t)imm, 2), 16));
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw, I,
          R(rd) = Mr((sword_t)src1 + (sword_t)imm, 4));
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu, I,
          R(rd) = Mr((sword_t)src1 + (sword_t)imm, 1));
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu, I,
          R(rd) = Mr((sword_t)src1 + (sword_t)imm, 2));
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb, S,
          Mw((sword_t)src1 + (sword_t)imm, 1, src2));
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh, S,
          Mw((sword_t)src1 + (sword_t)imm, 2, src2));
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw, S,
          Mw((sword_t)src1 + (sword_t)imm, 4, src2));
